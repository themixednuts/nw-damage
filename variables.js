//@ts-check

/**
 * @type {import("@mixednuts/types").WeaponItemDefinitions}
 */
export const wepDef = {
    "WeaponID": "SwordT5",
    "BaseWeaponID": "",
    "DEV Prio": 3,
    "PrimaryUse": "Combat Melee",
    "IconPath": "weapon_ironrapier",
    "MaxStackSize": 1,
    "EquipType": "RSword_LeftHolster",
    "DamageStatMultiplier": "OneHandSwordDamage",
    "WeaponMasteryCategoryId": "SwordAbilityTable",
    "TierNumber": 2,
    "BaseDamage": 64,
    "CritChance": 0.07,
    "CritDamageMultiplier": 1.3,
    "BaseStaggerDamage": 36,
    "CritStaggerDamageMultiplier": 1.5,
    "ReticleName": "melee",
    "ReticleTargetName": "spearHaveTarget",
    "ReticleRayCastDistance": 3,
    "AmmoType": "",
    "MaxLoadedAmmo": null,
    "AutoReloadAmmoSeconds": null,
    "AmmoMesh": "",
    "MannequinTag": "1H_Melee, Sword",
    "OffHandMannequinTag": "Melee_OH",
    "MeshOverride": "Objects\\Weapons\\Melee\\Swords\\1h\\StraightSwordA\\wep_me1h_sw_straightsworda_t5.cdf",
    "SkinOverride1": "",
    "MaterialOverride1": "",
    "SkinOverride2": "",
    "MaterialOverride2": "",
    "SkinOverride3": "",
    "MaterialOverride3": "",
    "SkinOverride4": "",
    "MaterialOverride4": "",
    "FireJoint": "",
    "DamageTableRow": "",
    "TooltipPrimaryAttackData": "1H_Sword_Attack1",
    "TooltipAlternateAttackData": "1H_Sword_Attack2",
    "AnimDbPath": "Animations/Mannequin/ADB/PlayerMaleAnims_1H_MeleeAttach.adb",
    "GatheringTypes": "None",
    "GatheringMultiplier": 1,
    "GatheringEfficiency": 0,
    "MinGatherEFF": null,
    "MaxGatherEFF": null,
    "AudioPickup": "Play_Beep1_2D",
    "AudioPlace": "Play_Beep3_2D",
    "Primary Hand": "Main",
    "EquipmentCategories": "",
    "RequiredStrength": 0,
    "RequiredDexterity": 0,
    "RequiredIntelligence": 0,
    "RequiredFocus": 0,
    "ScalingStrength": 0.009,
    "ScalingDexterity": 0.0065,
    "ScalingIntelligence": 0,
    "ScalingFocus": 0,
    "Resistances": "",
    "Weaknesses": "",
    "StatBonuses": "",
    "StatMultipliers": "",
    "EquipmentCategoryMultiplier": "",
    "AttackGameEventID": "WeaponHit5",
    "PhysicalArmorSetScaleFactor": null,
    "ElementalArmorSetScaleFactor": null,
    "ArmorRatingScaleFactor": null,
    "WeightOverride": null,
    "BlockStaminaDamage": 36,
    "BlockStability": 20,
    "DeflectionRating": 1,
    "BLAStandard": "1",
    "BLASiege": 1,
    "BLAStrike": 1,
    "BLASlash": 1,
    "BLAThrust": 1,
    "BLAArcane": 1,
    "BLAFire": 1,
    "BLAIce": 1,
    "BLANature": 1,
    "BLALightning": 1,
    "BLACorruption": 1,
    "ABAPoison": 0.4,
    "ABADisease": 0.4,
    "ABABleed": 0.4,
    "ABAFrostbite": 0.4,
    "ABACurse": 0.4,
    "RangedAttackProfile": "",
    "AttachedSpellData": "",
    "Appearance": "",
    "FemaleAppearance": "",
    "CanBlockRanged": null,
    "ManaCostId": "SpecialAttack",
    "WeaponEffectId": "Sword",
    "BaseAccuracy": 89,
    "SoundTableID": "1H_Sword"
}


/**
 * @type {import("@mixednuts/types/datasheets").ArmorItemDefinitions}
 */

export const armorDef = {
    "WeaponID": "LightChestT1",
    "Grit": 0,
    "PhysicalArmorSetScaleFactor": 0.25,
    "ElementalArmorSetScaleFactor": 0.25,
    "ArmorRatingScaleFactor": 0.3,
    "WeightOverride": 23.25,
    "Encumbrance": null,
    "EncumbrancePerGS": null,
    "WeightClass": "Light",
    "EquipLoad": "",
    "MaxHealthMod": "",
    "HealthRate": "",
    "MaxStaminaMod": "",
    "StaminaRate": "",
    "MaxManaMod": "",
    "ManaRate": "",
    "MaxFoodMod": "",
    "FoodBurn": "",
    "MaxDrinkMod": "",
    "DrinkBurn": "",
    "BONLogging": "",
    "BONMining": "",
    "BONSkinning": "",
    "BONHarvesting": "",
    "EFFLogging": "",
    "EFFMining": "",
    "EFFSkinning": "",
    "EFFHarvesting": "",
    "ROLLogging": "",
    "ROLMining": "",
    "ROLSkinning": "",
    "ROLHarvesting": "",
    "ROLAlchemy": "",
    "ROLBlacksmithing": "",
    "ROLEngineering": "",
    "ROLOutfitting": "",
    "ROLProvisioning": "",
    "ROLCamping": "",
    "DEFStandard": 0,
    "DEFSiege": 0,
    "DEFStrike": 0,
    "DEFSlash": 0,
    "DEFThrust": 0,
    "DEFMagic": 0,
    "DEFFire": 0,
    "DEFLightning": 0,
    "DEFCorruption": 0,
    "ABSStandard": 0,
    "ABSSiege": 0,
    "ABSStrike": 0,
    "ABSSlash": 0,
    "ABSThrust": 0,
    "ABSMagic": 0,
    "ABSFire": 0,
    "ABSLightning": 0,
    "ABSCorruption": 0,
    "RESPoison": 0,
    "RESDisease": 0,
    "RESBleed": 0,
    "RESFrostbite": 0,
    "RESCurse": 0,
    "BLAStandard": "",
    "BLASiege": "",
    "BLAStrike": "",
    "BLASlash": "",
    "BLAThrust": "",
    "BLAMagic": "",
    "BLAFire": "",
    "BLALightning": "",
    "BLACorruption": "",
    "ABAPoison": "",
    "ABADisease": "",
    "ABABleed": "",
    "ABAFrostbite": "",
    "ABACurse": "",
    "DMGStandard": "",
    "DMGSiege": "",
    "DMGStrike": "",
    "DMGSlash": "",
    "DMGThrust": "",
    "DMGMagic": "",
    "DMGFire": "",
    "DMGLightning": "",
    "DMGCorruption": "",
    "DMGVitalsCategory": "",
    "AFAPoison": "",
    "AFADisease": "",
    "AFABleed": "",
    "AFAFrostbite": "",
    "AFACurse": ""
}



/**
 * @type {import("@mixednuts/types/datasheets").MasterItemDefinitions}
 */
export const masterItem_heavyFeet = {
    "ItemID": "Omega_HeavyFeet_Soldier",
    "Name": "@Omega_HeavyFeet_MasterName",
    "ItemType": "Armor",
    "ItemTypeDisplayName": "@ui_itemtypedescription_heavy_feet_slot",
    "Description": "@Omega_HeavyFeet_Description",
    "ItemClass": "EquippableFeet+Armor+Heavy",
    "TradingCategory": "",
    "TradingFamily": "",
    "TradingGroup": "",
    "BindOnPickup": 1,
    "BindOnEquip": 0,
    "GearScoreOverride": 600,
    "MinGearScore": 600,
    "MaxGearScore": 600,
    "Tier": 5,
    "ItemStatsRef": "HeavyFeet_Balanced",
    "GrantsHWMBump": "",
    "IgnoreNameChanges": "",
    "IgnoreHWMScaling": "",
    "CanHavePerks": 1,
    "CanReplaceGem": 1,
    "Perk1": "PerkID_Stat_ArmorSoldier",
    "Perk2": "PerkID_Common_CDR",
    "Perk3": "PerkID_Gem_EmptyGemSlot",
    "Perk4": "",
    "Perk5": "",
    "PerkBucket1": "",
    "PerkBucket2": "",
    "PerkBucket3": "",
    "PerkBucket4": "",
    "PerkBucket5": "",
    "ForceRarity": null,
    "RequiredLevel": "",
    "UseTypeAffix": 1,
    "UseMaterialAffix": 1,
    "UseMagicAffix": 1,
    "UiItemClass": "UI_Armor",
    "ArmorAppearanceM": "Metal_Boots_T5",
    "ArmorAppearanceF": "F_Metal_Boots_T5",
    "WeaponAppearanceOverride": "",
    "ConfirmDestroy": "",
    "ConfirmBeforeUse": 0,
    "ConsumeOnUse": 0,
    "PrefabPath": "",
    "HousingTags": "",
    "IconPath": "",
    "HiResIconPath": "",
    "MaxStackSize": 1,
    "DeathDropPercentage": 0,
    "Nonremovable": null,
    "IsMissionItem": "",
    "IsUniqueItem": null,
    "ContainerLevel": null,
    "ContainerGS": "",
    "ExceedMinIndex": "",
    "ExceedMaxIndex": "",
    "IsSalvageable": 1,
    "SalvageResources": null,
    "IsRepairable": 1,
    "RepairDustModifier": 4,
    "RepairRecipe": "SalvageSmall_MetalT5",
    "CraftingRecipe": "SalvageSmall_MetalT5",
    "RepairGameEventID": "",
    "SalvageGameEventID": "SalvageCoinGrantT5",
    "SalvageAchievement": "",
    "RepairTypes": "",
    "IngredientCategories": "",
    "IngredientBonusPrimary": "",
    "IngredientBonusSecondary": "",
    "IngredientGearScoreBaseBonus": "",
    "IngredientGearScoreMaxBonus": "",
    "ExtraBonusItemChance": "",
    "Durability": 750,
    "DurabilityDmgOnDeath": 0.12,
    "DestroyOnBreak": 0,
    "Weight": 60,
    "AcquisitionNotificationId": "",
    "AudioPickup": "Play_UI_Pickup_Armour",
    "AudioPlace": "Play_UI_Drop_Armour",
    "AudioUse": "Play_UI_Use_Armour",
    "AudioCreated": "Play_UI_Created_Armour",
    "AudioDestroyed": "Play_UI_item_broken",
    "MannequinTag": "",
    "SoundTableID": "",
    "WarboardGatherStat": "",
    "WarboardDepositStat": "",
    "Notes": "",
    "HideInLootTicker": "",
    "HideFromRewardOpenPopup": null
}


/**
 * @type {import("@mixednuts/types/datasheets").MasterItemDefinitions}
 */
export const masterItem_fireStaff = {
    "ItemID": "Omega_StaffFire_Damage",
    "Name": "@Omega_StaffFire_MasterName",
    "ItemType": "Weapon",
    "ItemTypeDisplayName": "@ui_itemtypedescription_firestaff",
    "Description": "@Omega_StaffFire_Description",
    "ItemClass": "Weapon+EquippableMainHand+FireStaff+Magic",
    "TradingCategory": "",
    "TradingFamily": "",
    "TradingGroup": "",
    "BindOnPickup": 1,
    "BindOnEquip": 0,
    "GearScoreOverride": 600,
    "MinGearScore": 600,
    "MaxGearScore": 600,
    "Tier": 5,
    "ItemStatsRef": "FireStaffT5",
    "GrantsHWMBump": "",
    "IgnoreNameChanges": "",
    "IgnoreHWMScaling": "",
    "CanHavePerks": 1,
    "CanReplaceGem": 1,
    "Perk1": "PerkID_Stat_TwoHandScholar",
    "Perk2": "PerkID_Weapon_EmpowerOnCrit",
    "Perk3": "PerkID_Gem_EmptyGemSlot",
    "Perk4": "PerkID_Weapon_DmgBasic",
    "Perk5": "",
    "PerkBucket1": "",
    "PerkBucket2": "",
    "PerkBucket3": "",
    "PerkBucket4": "",
    "PerkBucket5": "",
    "ForceRarity": null,
    "RequiredLevel": "",
    "UseTypeAffix": 1,
    "UseMaterialAffix": 1,
    "UseMagicAffix": 1,
    "UiItemClass": "UI_Weapon",
    "ArmorAppearanceM": "",
    "ArmorAppearanceF": "",
    "WeaponAppearanceOverride": "2hfirestaffCrystallineT5",
    "ConfirmDestroy": "",
    "ConfirmBeforeUse": 0,
    "ConsumeOnUse": 0,
    "PrefabPath": "Items/Weapons/weapon",
    "HousingTags": "",
    "IconPath": "",
    "HiResIconPath": "",
    "MaxStackSize": 1,
    "DeathDropPercentage": 0,
    "Nonremovable": null,
    "IsMissionItem": "",
    "IsUniqueItem": null,
    "ContainerLevel": null,
    "ContainerGS": "",
    "ExceedMinIndex": "",
    "ExceedMaxIndex": "",
    "IsSalvageable": 1,
    "SalvageResources": null,
    "IsRepairable": 1,
    "RepairDustModifier": 4,
    "RepairRecipe": "SalvageLarge_MetalT5",
    "CraftingRecipe": "SalvageLarge_MetalT5",
    "RepairGameEventID": "",
    "SalvageGameEventID": "SalvageCoinGrantT5",
    "SalvageAchievement": "",
    "RepairTypes": "",
    "IngredientCategories": "",
    "IngredientBonusPrimary": "",
    "IngredientBonusSecondary": "",
    "IngredientGearScoreBaseBonus": "",
    "IngredientGearScoreMaxBonus": "",
    "ExtraBonusItemChance": "",
    "Durability": 3000,
    "DurabilityDmgOnDeath": 0.12,
    "DestroyOnBreak": 0,
    "Weight": 128,
    "AcquisitionNotificationId": "",
    "AudioPickup": "Play_UI_Pickup_Generic",
    "AudioPlace": "Play_UI_Drop_Generic",
    "AudioUse": "Play_UI_Use_Generic",
    "AudioCreated": "Play_UI_Created_Generic",
    "AudioDestroyed": "Play_UI_item_broken",
    "MannequinTag": "",
    "SoundTableID": "",
    "WarboardGatherStat": "",
    "WarboardDepositStat": "",
    "Notes": "",
    "HideInLootTicker": "",
    "HideFromRewardOpenPopup": null
}

/**
 * @type {import("@mixednuts/types/datasheets").MasterItemDefinitions}
 */
export const masterItem_sword = {
    "ItemID": "Omega_Longsword_Damage",
    "Name": "@Omega_Longsword_MasterName",
    "ItemType": "Weapon",
    "ItemTypeDisplayName": "@ui_itemtypedescription_sword",
    "Description": "@Omega_Longsword_Description",
    "ItemClass": "Weapon+EquippableMainHand+Sword+Melee",
    "TradingCategory": "",
    "TradingFamily": "",
    "TradingGroup": "",
    "BindOnPickup": 1,
    "BindOnEquip": 0,
    "GearScoreOverride": 600,
    "MinGearScore": 600,
    "MaxGearScore": 600,
    "Tier": 5,
    "ItemStatsRef": "SwordT5",
    "GrantsHWMBump": "",
    "IgnoreNameChanges": "",
    "IgnoreHWMScaling": "",
    "CanHavePerks": 1,
    "CanReplaceGem": 1,
    "Perk1": "PerkID_Stat_OneHandSoldier",
    "Perk2": "PerkID_Weapon_EmpowerOnCrit",
    "Perk3": "PerkID_Gem_EmptyGemSlot",
    "Perk4": "PerkID_Weapon_DmgBasic",
    "Perk5": "",
    "PerkBucket1": "",
    "PerkBucket2": "",
    "PerkBucket3": "",
    "PerkBucket4": "",
    "PerkBucket5": "",
    "ForceRarity": null,
    "RequiredLevel": "",
    "UseTypeAffix": 1,
    "UseMaterialAffix": 1,
    "UseMagicAffix": 1,
    "UiItemClass": "UI_Weapon",
    "ArmorAppearanceM": "",
    "ArmorAppearanceF": "",
    "WeaponAppearanceOverride": "1hSwordCrystallineT5",
    "ConfirmDestroy": "",
    "ConfirmBeforeUse": 0,
    "ConsumeOnUse": 0,
    "PrefabPath": "Items/Weapons/weapon",
    "HousingTags": "",
    "IconPath": "",
    "HiResIconPath": "",
    "MaxStackSize": 1,
    "DeathDropPercentage": 0,
    "Nonremovable": null,
    "IsMissionItem": "",
    "IsUniqueItem": null,
    "ContainerLevel": null,
    "ContainerGS": "",
    "ExceedMinIndex": "",
    "ExceedMaxIndex": "",
    "IsSalvageable": 1,
    "SalvageResources": null,
    "IsRepairable": 1,
    "RepairDustModifier": 4,
    "RepairRecipe": "SalvageSmall_MetalT5",
    "CraftingRecipe": "SalvageSmall_MetalT5",
    "RepairGameEventID": "",
    "SalvageGameEventID": "SalvageCoinGrantT5",
    "SalvageAchievement": "",
    "RepairTypes": "",
    "IngredientCategories": "",
    "IngredientBonusPrimary": "",
    "IngredientBonusSecondary": "",
    "IngredientGearScoreBaseBonus": "",
    "IngredientGearScoreMaxBonus": "",
    "ExtraBonusItemChance": "",
    "Durability": 3000,
    "DurabilityDmgOnDeath": 0.12,
    "DestroyOnBreak": 0,
    "Weight": 54,
    "AcquisitionNotificationId": "",
    "AudioPickup": "Play_UI_Pickup_Sword",
    "AudioPlace": "Play_UI_Drop_Sword",
    "AudioUse": "Play_UI_Use_Sword",
    "AudioCreated": "Play_UI_Created_Sword",
    "AudioDestroyed": "Play_UI_item_broken",
    "MannequinTag": "",
    "SoundTableID": "",
    "WarboardGatherStat": "",
    "WarboardDepositStat": "",
    "Notes": "",
    "HideInLootTicker": "",
    "HideFromRewardOpenPopup": null
}

/**
 * @type {Map<number, import("@mixednuts/types/datasheets").AttributeDefinition}
 */
export const attributeDefinitions = new Map()
attributeDefinitions.set(5, {
    "Level": 5,
    "Health": 0,
    "HealthRate": 0,
    "Stamina": 0,
    "StaminaRate": 0,
    "Mana": 0,
    "ManaRate": 0,
    "CastSpeed": 0,
    "EquipLoad": 0,
    "NumAttuneSlots": 0,
    "Encumbrance": 0,
    "AbsFalling": 0,
    "DefStandard": 0,
    "DefSlash": 0,
    "DefThrust": 0,
    "DefStrike": 0,
    "DefMagic": 0,
    "DefFire": 0,
    "DefLightning": 0,
    "DefCorruption": 0,
    "ResFrostbite": 0,
    "ResPoison": 0,
    "ResBleed": 0,
    "ResDisease": 0,
    "ResCurse": 0,
    "ScalingValue": 0,
    "ModifierValue": 1.625,
    "ModifierValueSum": 0,
    "EquipAbilities": ""
})
attributeDefinitions.set(500, {
    "Level": 500,
    "Health": 0,
    "HealthRate": 0,
    "Stamina": 0,
    "StaminaRate": 0,
    "Mana": 0,
    "ManaRate": 0,
    "CastSpeed": 0,
    "EquipLoad": 0,
    "NumAttuneSlots": 0,
    "Encumbrance": 0,
    "AbsFalling": 0,
    "DefStandard": 0,
    "DefSlash": 0,
    "DefThrust": 0,
    "DefStrike": 0,
    "DefMagic": 0,
    "DefFire": 0,
    "DefLightning": 0,
    "DefCorruption": 0,
    "ResFrostbite": 0,
    "ResPoison": 0,
    "ResBleed": 0,
    "ResDisease": 0,
    "ResCurse": 0,
    "ScalingValue": 0,
    "ModifierValue": 0.78,
    "ModifierValueSum": 531.375,
    "EquipAbilities": ""
})

/**
 * @type {import("@mixednuts/types/datasheets").PerkData}
 */
export const fireGem_t2 = {
    "PerkID": "PerkID_Gem_FireDMG2",
    "PerkType": "Gem",
    "ConditionEvent": "OnEquip",
    "ItemClass": "EquippableMainHand+EquippableTwoHand",
    "ExcludeItemClass": "FireStaff",
    "Channel": 4,
    "ExclusiveLabels": "Lightning+Arcane+Void+Ice+Nature+Gem",
    "ItemPerkConflictsLocText": "@incompatible_elemental_perk",
    "DeprecatedPerkId": "",
    "ScalingPerGearScore": null,
    "ItemClassGSBonus": "",
    "DisplayName": "@FireDMG2Name",
    "Description": "@FireDMG2Desc",
    "GroupName": "Ruby_FireDMG",
    "Tier": 2,
    "ExcludeFromTradingPost": "",
    "AppliedPrefix": "@IgnitedPrefix",
    "AppliedSuffix": "",
    "NamePriority": "",
    "IconPath": "LyShineUI/Images/Icons/Items/Resource/RubyCutT2.png",
    "Affix": "Ignited2",
    "EquipAbility": "",
    "DayPhases": "",
    "FishingWaterType": "",
    "TerritoryId": "",
    "CombatStatus": "",
    "WeaponTag": "",
    "Category": ""
}
